	...
	%>
		...
		<div class="main-content ">
			<div class="col-md-12">
				<div class="row">
						...
						<form id="selectionForm" method="post">
							<div
								...
								<div class="report-info">
									...
									<select name="userId" class="form-select form-select-sm" id="users-drop-down">
										<option value="0" selected="selected">Select
											User</option>
									</select>
								</div>
								...
								<div class="report-info">
									<div class="form-text">Task</div>
									<select name="taskStatus" class="form-select form-select-sm" id="status-drop-down">
										<option value="0" selected="selected">Select
											Task Status</option>
									</select>
								</div>
								...
								<div class="report-info">
									...
									<select name="timespan" class="form-select form-select-sm" id="time-drop-down"
								...
								<div class="report-info">
									...
									<button type="button" class="btn btn-primary btn-sm " onclick="generateJSON()"
										id="generate-report">Generate
										Report</button>

									<div class="dropdown" style="display: initial" id="report-button-dropdown">
										<button class="btn btn-primary btn-sm dropdown-toggle" type="button"
											id="dropdownMenuButton1" data-bs-toggle="dropdown"
											aria-expanded="false">Download Report</button>
									...
									<button type="button" class="btn btn-primary btn-sm " onclick="downloadJSON()"
										id="email-report" style="display: none;">Send Email</button>
										
									<button type="button" class="btn btn-primary btn-sm " onclick="sendEmail('${pageContext.request.contextPath}')"
										id="generate-report">Send Mail</button>										
								</div>
							</div>
					...
					<div>
						<table class="table-new " id="tableID" style="max-width: 100% !important; display: none;">
							<thead>
		...
		<script>
			...
			function downloadExcel (basePath) {
				...
				form.action = basePath + "/report/download/excel";
				form.submit();
			}
			...
			function sendEmail (basePath ) {
				$.ajax({
				      type: "POST",
				      url: basePath + "/report/download/send-email",
			...
			function downloadPdf (basePath ) {
				let form = document.getElementById( 'selectionForm' );
				form.action = basePath + "/report/download/pdf";
				form.submit();
			}
			...
			function generateJSON () {
				...
				$.ajax( {
					type: 'POST',
					url: '${pageContext.request.contextPath}/reports/tasks',
					data: jsonString,
